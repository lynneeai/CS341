...
class ScreenQuad {
    ...
    public:
        ...
        void UpdateBlurType(int blur) {
            blur_type = blur;
            if (blur == 1) {
                std::cout << "new blur_type: brute" << std::endl;
            }
            else {
                std::cout << "new blur_type: fast" << std::endl;
            }
        }
        ...
        void Draw(PASS::ENUM pass_nb) {
            ...
            glUniform1f(glGetUniformLocation(program_id_, "tex_height"),
                        screenquad_height_);
            glUniform1i(glGetUniformLocation(program_id_, "blur_type"), blur_type);
